...
async def git(*arguments, cwd=None, timeout=30, lock_fd=None):
    try:
        code, output = await run(
            ["git", "-c", "core.hooksPath=/dev/null", "-c", "protocol.ext.allow=never", *arguments],
            cwd=cwd, timeout=timeout, env=environment(), lock_fd=lock_fd,
        )
...
async def checkout(config, project, sha, paths, ident, job_id, lock_fd=None):
    ...
    if not cache.exists():
        await git("init", "--bare", f"--object-format={'sha256' if len(sha) == 64 else 'sha1'}",
                  cache, lock_fd=lock_fd)
    ref = "refs/heads/autocd-candidate"
    await git("--git-dir", cache, "fetch", "--no-tags", "--depth=1", "--force", url,
              f"+refs/heads/{config.branch}:{ref}", timeout=120, lock_fd=lock_fd)
    fetched = await git("--git-dir", cache, "rev-parse", ref, lock_fd=lock_fd)
    if fetched != sha:
    ...
    await git("clone", "--no-hardlinks", "--no-checkout", "--branch", "autocd-candidate",
              cache, release, timeout=120, lock_fd=lock_fd)
    await git("checkout", "--detach", sha, cwd=release, timeout=120, lock_fd=lock_fd)
    return release
