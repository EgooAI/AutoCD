...
async def run(argv, *, timeout=30, cwd=None, env=None, output=None, input_data=None, pass_fds=()):
    try:
...
async def check_bash(script, pass_fds=()):
    environment = os.environ.copy()
    environment.pop("BASH_ENV", None)
    environment.pop("ENV", None)
    code, _ = await run(["bash", "--noprofile", "--norc", "-n"],
                        input_data=script.encode(), env=environment, pass_fds=pass_fds)
    if code:
